...
with lib; let
  ...
  extractPreviewers = p:
    if p.previewers != null
    then p.previewers # User override
    else p.plugin.yaziPreviewers or []; # Plugin metadata

  extractPreloaders = p:
    if p.preloaders != null
    then p.preloaders # User override
    else p.plugin.yaziPreloaders or []; # Plugin metadata
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    programs.yazi.initLua = mkIf (initLuaContent != "") (
      mkBefore initLuaContent
    );
